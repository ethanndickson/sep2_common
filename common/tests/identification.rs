...
fn default_Resource() {
    ...
    let new: Resource = from_str(&to_string_with_config(&orig, &YASERDE_CFG).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Response() {
    ...
    let new: Response = from_str(&to_string_with_config(&orig, &YASERDE_CFG).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ListLink() {
    ...
    let new: ListLink = from_str(&to_string_with_config(&orig, &YASERDE_CFG).unwrap()).unwrap();
    assert_eq!(orig, new);
}
