...
fn default_DeviceCapability() {
    ...
    let new: DeviceCapability = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AbstractDevice() {
    ...
    let new: AbstractDevice = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DeviceStatus() {
    ...
    let new: DeviceStatus = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_EndDevice() {
    ...
    let new: EndDevice = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_EndDeviceList() {
    ...
    let new: EndDeviceList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Registration() {
    ...
    let new: Registration = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SelfDevice() {
    ...
    let new: SelfDevice = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Temperature() {
    ...
    let new: Temperature = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FunctionSetAssignmentsBase() {
    ...
    let new: FunctionSetAssignmentsBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FunctionSetAssignments() {
    ...
    let new: FunctionSetAssignments = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FunctionSetAssignmentsList() {
    ...
    let new: FunctionSetAssignmentsList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Condition() {
    ...
    let new: Condition = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SubscriptionBase() {
    ...
    let new: SubscriptionBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Subscription() {
    ...
    let new: Subscription = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SubscriptionList() {
    ...
    let new: SubscriptionList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Notification() {
    ...
    let new: Notification<EndDeviceControl> = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_NotificationList() {
    ...
    let new: NotificationList<EndDeviceControl> = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERControlResponse() {
    ...
    let new: DERControlResponse = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowReservationResponseResponse() {
    ...
    let new: FlowReservationResponseResponse = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AppliedTargetReduction() {
    ...
    let new: AppliedTargetReduction = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DrResponse() {
    ...
    let new: DrResponse = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PriceResponse() {
    ...
    let new: PriceResponse = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ResponseSet() {
    ...
    let new: ResponseSet = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ResponseSetList() {
    ...
    let new: ResponseSetList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TextResponse() {
    ...
    let new: TextResponse = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Time() {
    ...
    let new: Time = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DeviceInformation() {
    ...
    let new: DeviceInformation = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DRLCCapabilities() {
    ...
    let new: DRLCCapabilities = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SupportedLocale() {
    ...
    let new: SupportedLocale = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SupportedLocaleList() {
    ...
    let new: SupportedLocaleList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PowerStatus() {
    ...
    let new: PowerStatus = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PowerSourceType() {
    ...
    let new: PowerSourceType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PEVInfo() {
    ...
    let new: Pevinfo = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_IPAddr() {
    ...
    let new: IPAddr = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_IPAddrList() {
    ...
    let new: IPAddrList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_IPInterface() {
    ...
    let new: IPInterface = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_IPInterfaceList() {
    ...
    let new: IPInterfaceList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LLInterfaceList() {
    ...
    let new: LlinterfaceList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_loWPAN() {
    ...
    let new: LoWPAN = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Neighbor() {
    ...
    let new: Neighbor = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_NeighborList() {
    ...
    let new: NeighborList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LogEvent() {
    ...
    let new: LogEvent = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LogEventList() {
    ...
    let new: LogEventList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Configuration() {
    ...
    let new: Configuration = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PowerConfiguration() {
    ...
    let new: PowerConfiguration = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PriceResponseCfg() {
    ...
    let new: PriceResponseCfg = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PriceResponseCfgList() {
    ...
    let new: PriceResponseCfgList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TimeConfiguration() {
    ...
    let new: TimeConfiguration = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_File() {
    ...
    let new: File = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FileList() {
    ...
    let new: FileList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FileStatus() {
    ...
    let new: FileStatus = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LoadShedAvailabilityList() {
    ...
    let new: LoadShedAvailabilityList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ApplianceLoadReduction() {
    ...
    let new: ApplianceLoadReduction = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DemandResponseProgramList() {
    ...
    let new: DemandResponseProgramList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DutyCycle() {
    ...
    let new: DutyCycle = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_EndDeviceControlList() {
    ...
    let new: EndDeviceControlList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LoadShedAvailability() {
    ...
    let new: LoadShedAvailability = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Offset() {
    ...
    let new: Offset = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SetPoint() {
    ...
    let new: SetPoint = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TargetReduction() {
    ...
    let new: TargetReduction = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MeterReading() {
    ...
    let new: MeterReading = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MeterReadingList() {
    ...
    let new: MeterReadingList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Reading() {
    ...
    let new: Reading = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingList() {
    ...
    let new: ReadingList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingSet() {
    ...
    let new: ReadingSet = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingSetList() {
    ...
    let new: ReadingSetList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingType() {
    ...
    let new: ReadingType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UsagePoint() {
    ...
    let new: UsagePoint = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UsagePointList() {
    ...
    let new: UsagePointList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ConsumptionTariffInterval() {
    ...
    let new: ConsumptionTariffInterval = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ConsumptionTariffIntervalList() {
    ...
    let new: ConsumptionTariffIntervalList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CostKindType() {
    ...
    let new: CostKindType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_EnvironmentalCost() {
    ...
    let new: EnvironmentalCost = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RateComponent() {
    ...
    let new: RateComponent = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RateComponentList() {
    ...
    let new: RateComponentList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TariffProfileList() {
    ...
    let new: TariffProfileList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TimeTariffIntervalList() {
    ...
    let new: TimeTariffIntervalList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MessagingProgramList() {
    ...
    let new: MessagingProgramList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PriorityType() {
    ...
    let new: PriorityType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TextMessageList() {
    ...
    let new: TextMessageList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingPeriod() {
    ...
    let new: BillingPeriod = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingPeriodList() {
    ...
    let new: BillingPeriodList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingMeterReadingBase() {
    ...
    let new: BillingMeterReadingBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingReading() {
    ...
    let new: BillingReading = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingReadingList() {
    ...
    let new: BillingReadingList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingReadingSet() {
    ...
    let new: BillingReadingSet = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingReadingSetList() {
    ...
    let new: BillingReadingSetList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Charge() {
    ...
    let new: Charge = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ChargeKind() {
    ...
    let new: ChargeKind = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CustomerAccount() {
    ...
    let new: CustomerAccount = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CustomerAccountList() {
    ...
    let new: CustomerAccountList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CustomerAgreement() {
    ...
    let new: CustomerAgreement = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CustomerAgreementList() {
    ...
    let new: CustomerAgreementList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_HistoricalReading() {
    ...
    let new: HistoricalReading = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_HistoricalReadingList() {
    ...
    let new: HistoricalReadingList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ProjectionReading() {
    ...
    let new: ProjectionReading = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ProjectionReadingList() {
    ...
    let new: ProjectionReadingList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TargetReading() {
    ...
    let new: TargetReading = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TargetReadingList() {
    ...
    let new: TargetReadingList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ServiceSupplier() {
    ...
    let new: ServiceSupplier = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ServiceSupplierList() {
    ...
    let new: ServiceSupplierList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AccountBalance() {
    ...
    let new: AccountBalance = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AccountingUnit() {
    ...
    let new: AccountingUnit = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CreditRegister() {
    ...
    let new: CreditRegister = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CreditRegisterList() {
    ...
    let new: CreditRegisterList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Prepayment() {
    ...
    let new: Prepayment = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PrepaymentList() {
    ...
    let new: PrepaymentList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PrepayModeType() {
    ...
    let new: PrepayModeType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PrepayOperationStatus() {
    ...
    let new: PrepayOperationStatus = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ServiceChange() {
    ...
    let new: ServiceChange = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SupplyInterruptionOverride() {
    ...
    let new: SupplyInterruptionOverride = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SupplyInterruptionOverrideList() {
    ...
    let new: SupplyInterruptionOverrideList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CreditStatusType() {
    ...
    let new: CreditStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CreditTypeType() {
    ...
    let new: CreditTypeType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CreditTypeChange() {
    ...
    let new: CreditTypeChange = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ServiceStatusType() {
    ...
    let new: ServiceStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RequestStatus() {
    ...
    let new: RequestStatus = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowReservationRequest() {
    ...
    let new: FlowReservationRequest = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowReservationRequestList() {
    ...
    let new: FlowReservationRequestList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowReservationResponse() {
    ...
    let new: FlowReservationResponse = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowReservationResponseList() {
    ...
    let new: FlowReservationResponseList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DefaultDERControl() {
    ...
    let new: DefaultDERControl = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FreqDroopType() {
    ...
    let new: FreqDroopType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DER() {
    ...
    let new: DER = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERList() {
    ...
    let new: DERList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERSettings() {
    ...
    let new: DERSettings = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERType() {
    ...
    let new: DERType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERAvailability() {
    ...
    let new: DERAvailability = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERCapability() {
    ...
    let new: DERCapability = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERControlBase() {
    ...
    let new: DERControlBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERControlList() {
    ...
    let new: DERControlList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERControlType() {
    ...
    let new: DERControlType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERCurve() {
    ...
    let new: DERCurve = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CurveData() {
    ...
    let new: CurveData = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERCurveType() {
    ...
    let new: DERCurveType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERProgram() {
    ...
    let new: DERProgram = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERProgramList() {
    ...
    let new: DERProgramList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERStatus() {
    ...
    let new: DERStatus = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERUnitRefType() {
    ...
    let new: DERUnitRefType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CurrentRMS() {
    ...
    let new: CurrentRMS = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FixedPointType() {
    ...
    let new: FixedPointType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UnsignedFixedPointType() {
    ...
    let new: UnsignedFixedPointType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActivePower() {
    ...
    let new: ActivePower = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AmpereHour() {
    ...
    let new: AmpereHour = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ApparentPower() {
    ...
    let new: ApparentPower = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReactivePower() {
    ...
    let new: ReactivePower = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReactiveSusceptance() {
    ...
    let new: ReactiveSusceptance = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PowerFactor() {
    ...
    let new: PowerFactor = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PowerFactorWithExcitation() {
    ...
    let new: PowerFactorWithExcitation = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FixedVar() {
    ...
    let new: FixedVar = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_WattHour() {
    ...
    let new: WattHour = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_VoltageRMS() {
    ...
    let new: VoltageRMS = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ConnectStatusType() {
    ...
    let new: ConnectStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_InverterStatusType() {
    ...
    let new: InverterStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LocalControlModeStatusType() {
    ...
    let new: LocalControlModeStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ManufacturerStatusType() {
    ...
    let new: ManufacturerStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_OperationalModeStatusType() {
    ...
    let new: OperationalModeStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_StateOfChargeStatusType() {
    ...
    let new: StateOfChargeStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_StorageModeStatusType() {
    ...
    let new: StorageModeStatusType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AccountBalanceLink() {
    ...
    let new: AccountBalanceLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveBillingPeriodListLink() {
    ...
    let new: ActiveBillingPeriodListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveCreditRegisterListLink() {
    ...
    let new: ActiveCreditRegisterListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveDERControlListLink() {
    ...
    let new: ActiveDERControlListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveEndDeviceControlListLink() {
    ...
    let new: ActiveEndDeviceControlListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveFlowReservationListLink() {
    ...
    let new: ActiveFlowReservationListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveProjectionReadingListLink() {
    ...
    let new: ActiveProjectionReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveTargetReadingListLink() {
    ...
    let new: ActiveTargetReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveTextMessageListLink() {
    ...
    let new: ActiveTextMessageListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ActiveTimeTariffIntervalListLink() {
    ...
    let new: ActiveTimeTariffIntervalListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AssociatedDERProgramListLink() {
    ...
    let new: AssociatedDERProgramListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AssociatedUsagePointLink() {
    ...
    let new: AssociatedUsagePointLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingPeriodListLink() {
    ...
    let new: BillingPeriodListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingReadingListLink() {
    ...
    let new: BillingReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_BillingReadingSetListLink() {
    ...
    let new: BillingReadingSetListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ConfigurationLink() {
    ...
    let new: ConfigurationLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ConsumptionTariffIntervalListLink() {
    ...
    let new: ConsumptionTariffIntervalListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CreditRegisterListLink() {
    ...
    let new: CreditRegisterListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CustomerAccountLink() {
    ...
    let new: CustomerAccountLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CustomerAccountListLink() {
    ...
    let new: CustomerAccountListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CustomerAgreementListLink() {
    ...
    let new: CustomerAgreementListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DemandResponseProgramLink() {
    ...
    let new: DemandResponseProgramLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DemandResponseProgramListLink() {
    ...
    let new: DemandResponseProgramListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERAvailabilityLink() {
    ...
    let new: DERAvailabilityLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DefaultDERControlLink() {
    ...
    let new: DefaultDERControlLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERCapabilityLink() {
    ...
    let new: DERCapabilityLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERControlListLink() {
    ...
    let new: DERControlListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERCurveLink() {
    ...
    let new: DERCurveLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERCurveListLink() {
    ...
    let new: DERCurveListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERLink() {
    ...
    let new: DERLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERListLink() {
    ...
    let new: DERListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERProgramLink() {
    ...
    let new: DERProgramLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERProgramListLink() {
    ...
    let new: DERProgramListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERSettingsLink() {
    ...
    let new: DERSettingsLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DERStatusLink() {
    ...
    let new: DERStatusLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DeviceCapabilityLink() {
    ...
    let new: DeviceCapabilityLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DeviceInformationLink() {
    ...
    let new: DeviceInformationLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DeviceStatusLink() {
    ...
    let new: DeviceStatusLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_EndDeviceControlListLink() {
    ...
    let new: EndDeviceControlListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_EndDeviceLink() {
    ...
    let new: EndDeviceLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_EndDeviceListLink() {
    ...
    let new: EndDeviceListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FileLink() {
    ...
    let new: FileLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FileListLink() {
    ...
    let new: FileListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FileStatusLink() {
    ...
    let new: FileStatusLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowReservationRequestListLink() {
    ...
    let new: FlowReservationRequestListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowReservationResponseListLink() {
    ...
    let new: FlowReservationResponseListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FunctionSetAssignmentsListLink() {
    ...
    let new: FunctionSetAssignmentsListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_HistoricalReadingListLink() {
    ...
    let new: HistoricalReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_IPAddrListLink() {
    ...
    let new: IPAddrListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_IPInterfaceListLink() {
    ...
    let new: IPInterfaceListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LLInterfaceListLink() {
    ...
    let new: LlinterfaceListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LoadShedAvailabilityListLink() {
    ...
    let new: LoadShedAvailabilityListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LogEventListLink() {
    ...
    let new: LogEventListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MessagingProgramListLink() {
    ...
    let new: MessagingProgramListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MeterReadingLink() {
    ...
    let new: MeterReadingLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MeterReadingListLink() {
    ...
    let new: MeterReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MirrorUsagePointListLink() {
    ...
    let new: MirrorUsagePointListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_NeighborListLink() {
    ...
    let new: NeighborListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_NotificationListLink() {
    ...
    let new: NotificationListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PowerStatusLink() {
    ...
    let new: PowerStatusLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PrepaymentLink() {
    ...
    let new: PrepaymentLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PrepaymentListLink() {
    ...
    let new: PrepaymentListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PrepayOperationStatusLink() {
    ...
    let new: PrepayOperationStatusLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PriceResponseCfgListLink() {
    ...
    let new: PriceResponseCfgListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ProjectionReadingListLink() {
    ...
    let new: ProjectionReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RateComponentLink() {
    ...
    let new: RateComponentLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RateComponentListLink() {
    ...
    let new: RateComponentListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingLink() {
    ...
    let new: ReadingLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingListLink() {
    ...
    let new: ReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingSetListLink() {
    ...
    let new: ReadingSetListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingTypeLink() {
    ...
    let new: ReadingTypeLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RegistrationLink() {
    ...
    let new: RegistrationLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ResponseListLink() {
    ...
    let new: ResponseListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ResponseSetListLink() {
    ...
    let new: ResponseSetListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RPLInstanceListLink() {
    ...
    let new: RplinstanceListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RPLSourceRoutesListLink() {
    ...
    let new: RplsourceRoutesListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SelfDeviceLink() {
    ...
    let new: SelfDeviceLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ServiceSupplierLink() {
    ...
    let new: ServiceSupplierLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SubscriptionListLink() {
    ...
    let new: SubscriptionListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SupplyInterruptionOverrideListLink() {
    ...
    let new: SupplyInterruptionOverrideListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SupportedLocaleListLink() {
    ...
    let new: SupportedLocaleListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TargetReadingListLink() {
    ...
    let new: TargetReadingListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TariffProfileLink() {
    ...
    let new: TariffProfileLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TariffProfileListLink() {
    ...
    let new: TariffProfileListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TextMessageListLink() {
    ...
    let new: TextMessageListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TimeLink() {
    ...
    let new: TimeLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TimeTariffIntervalListLink() {
    ...
    let new: TimeTariffIntervalListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UsagePointLink() {
    ...
    let new: UsagePointLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UsagePointListLink() {
    ...
    let new: UsagePointListLink = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_AccumulationBehaviourType() {
    ...
    let new: AccumulationBehaviourType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ApplianceLoadReductionType() {
    ...
    let new: ApplianceLoadReductionType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CommodityType() {
    ...
    let new: CommodityType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ConsumptionBlockType() {
    ...
    let new: ConsumptionBlockType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_CurrencyCode() {
    ...
    let new: CurrencyCode = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DataQualifierType() {
    ...
    let new: DataQualifierType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DateTimeInterval() {
    ...
    let new: DateTimeInterval = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DeviceCategoryType() {
    ...
    let new: DeviceCategoryType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_DstRuleType() {
    ...
    let new: DstRuleType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_FlowDirectionType() {
    ...
    let new: FlowDirectionType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_KindType() {
    ...
    let new: KindType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_LocaleType() {
    ...
    let new: LocaleType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_mRIDType() {
    ...
    let new: MRIDType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_OneHourRangeType() {
    ...
    let new: OneHourRangeType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PENType() {
    ...
    let new: PENType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_Percent() {
    ...
    let new: Percent = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PhaseCode() {
    ...
    let new: PhaseCode = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PINType() {
    ...
    let new: PINType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_PowerOfTenMultiplierType() {
    ...
    let new: PowerOfTenMultiplierType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RealEnergy() {
    ...
    let new: RealEnergy = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_RoleFlagsType() {
    ...
    let new: RoleFlagsType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ServiceKind() {
    ...
    let new: ServiceKind = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SFDIType() {
    ...
    let new: SFDIType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SignedPercent() {
    ...
    let new: SignedPercent = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_SignedRealEnergy() {
    ...
    let new: SignedRealEnergy = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TimeOffsetType() {
    ...
    let new: TimeOffsetType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_TimeType() {
    ...
    let new: TimeType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UnitType() {
    ...
    let new: UnitType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UnitValueType() {
    ...
    let new: UnitValueType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UomType() {
    ...
    let new: UomType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_VersionType() {
    ...
    let new: VersionType = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MirrorMeterReading() {
    ...
    let new: MirrorMeterReading = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MirrorMeterReadingList() {
    ...
    let new: MirrorMeterReadingList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MeterReadingBase() {
    ...
    let new: MeterReadingBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MirrorReadingSet() {
    ...
    let new: MirrorReadingSet = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MirrorUsagePoint() {
    ...
    let new: MirrorUsagePoint = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_MirrorUsagePointList() {
    ...
    let new: MirrorUsagePointList = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingBase() {
    ...
    let new: ReadingBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_ReadingSetBase() {
    ...
    let new: ReadingSetBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
...
fn default_UsagePointBase() {
    ...
    let new: UsagePointBase = from_str(&to_string(&orig).unwrap()).unwrap();
    assert_eq!(orig, new);
}
